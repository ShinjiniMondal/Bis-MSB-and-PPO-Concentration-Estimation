...
calib_concs = np.array([
    ufloat(0.5013424359375951, 0.014124117584641807),
    ufloat(4.133930067073276, 0.11644902010082281),
    ufloat(7.747769219327349, 0.21824716028141486),
    ufloat(11.365453821377393, 0.32015370146284916),
    ufloat(15.021776594012007, 0.42314875138913116),
    ufloat(17.001894052862475, 0.4789266662232216),
    ufloat(19.046352579807362, 0.5365170925199219)
])

# Calibration areas
calib_areas = np.array([1.2583197838442477, 12.55875568379027, 23.929424472774066,
                        33.70109321891018, 45.4515430117026, 51.260019147458166, 56.473285756073885])
...
integration_params = {
    ...
    "end_wl": 400.0,
    "method": "simpson",
    "note": "bis-MSB and PPO sample integration range for concentration estimation of unknown sample. This works for samples having 3.5 g/L concentration of PPO and unknown concentration of bis-MSB. The reference sample for UV-Vis measurements is 3.5 g/L PPO."
}
...
def main():
    if len(sys.argv) != 6:
        print("Usage: python bisMSB and PPO_calculate_concentration.py <file.spc> <start_wl> <end_wl> <norm_wl>")
        sys.exit(1)
    ...
    print(f"Estimated Concentration = 3.5 g/L PPO and {C_with_uncertainty:.5uP} mg/L bis-MSB")
    print(f"Uncertainty Breakdown (variance contributions):")
    print(f"  From slope: {contrib_slope:.5f} g/L ({perc_slope: 2f}%)")
    print(f"  From area integration: {contrib_area:.5f} g/L ({perc_area:.2f}%)")
    print(f"  From calibration residuals: {contrib_res:.5f} g/L ({perc_res:.2f}%)")
    print(f"Total combined uncertainty: {total_sigma:.5f} g/L")
